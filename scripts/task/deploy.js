...
module.exports = function(distDir, deployWebappDir, deployLibDir, gulp) {
  ...
  gulp.task('clean-deploy', gulp.series('build', function(cb) {
  ...
  gulp.task('copy-dist', gulp.series('clean-deploy', function(cb) {
    gulp.src([distDir, 'webapps', '**', '*'].join(path.sep))
        .pipe(gulp.dest(deployWebappDir))
        .on('end', cb);
  }));

  gulp.task('copy-libs', gulp.series('copy-dist', function(cb) {
